...
class BurpExtender(IBurpExtender, IScannerInsertionPointProvider):
    ...
    def	registerExtenderCallbacks(self, callbacks):
        ...
        callbacks.setExtensionName("Scan Replace Insertion")
...
class InsertionPoint(IScannerInsertionPoint):
    ...
    def buildRequest(self, payload):
        ...
        if self._parameter.getType()==IParameter.PARAM_URL:
            newParameValue=self._helpers.bytesToString(self._helpers.urlEncode(payload))
        else:
            newParameValue=self._helpers.bytesToString(payload)
